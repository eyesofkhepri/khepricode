...
```javascript
create database db_flyway_version;
create user 'fly'@'%' identified by 'pray';
grant all privileges on *.* to 'fly'@'%';
grant all privileges on db_flyway_version.* to 'fly'@'%';
```
...
메이븐 센트럴 레파지토리에서는 jooq open source만 사용이 가능합니다. jooq Professional 버전을 사용하실 꺼라면 내부 Nexus에 올려서 사용하거나 로컬 Maven 캐시에 수동으로 등록하세요.
